...
//     - 任务启动后默认启用，并立即执行第一轮自检；
//     - 通过 setEnabled / setPeriod 控制后续启停与周期；
...
private:
    // ---- 配置 ----
    bool      m_enabled    = true;      // 是否启用；常驻任务启动后立即执行第一轮
    ...
    QHash<QString, SH85SelfCheckTaskRecord> m_deviceRecords; // 每设备聚合日志记录

    // ---- 信号连接（用于 stop/解除）----
    QHash<QString, SH85SelfChecker::State> m_checkerStates;  // Per-device checker state for command records.
