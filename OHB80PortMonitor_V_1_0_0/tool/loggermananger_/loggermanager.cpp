...
namespace {
spdlog::async_overflow_policy overflow_policy_for_file(const std::string& filename)
{
    (void)filename;
    // Never block producer threads. Under pressure we prefer dropping oldest logs
    // over freezing scheduler/UI threads.
      ...
      cleanup_running_(true)
{
    ...
    spdlog::init_thread_pool(262144, 4);
    ...
    const std::string& filename)
{
    ...
    const auto overflow_policy = overflow_policy_for_file(filename);
    ...
    const std::string& date_str)
{
    ...
    // 第二阶段：写锁创建缺失的 loggers
    {
        ...
        for (size_t idx : missing_indices) {
            ...
            spdlog::drop(logger_name);
            const auto overflow_policy = overflow_policy_for_file(targets[idx].filename);
