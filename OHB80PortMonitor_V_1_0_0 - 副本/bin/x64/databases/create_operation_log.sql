...
-- VEFC 传感器监控表
-- ============================================
CREATE TABLE IF NOT EXISTS vefc_sensor_monitor (
    qr_code TEXT NOT NULL,                                                  -- QRCode（设备标识）
    record_timestamp INTEGER NOT NULL,                                      -- 记录时间戳（毫秒）
    gas_pressure REAL NOT NULL,                                             -- 气体压力
    actual_flow REAL NOT NULL,                                              -- 实际流量
    sensor_pressure REAL NOT NULL,                                          -- 传感器压力
    sensor_temperature REAL NOT NULL,                                       -- 传感器温度
    PRIMARY KEY (qr_code, record_timestamp)
);

-- VEFC 传感器监控索引
-- 复合主键已覆盖按设备 + 时间戳查询
CREATE INDEX IF NOT EXISTS idx_vefc_sensor_monitor_record_timestamp ON vefc_sensor_monitor(record_timestamp);
