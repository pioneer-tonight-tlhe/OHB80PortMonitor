#ifndef NETWORK_STATUS_TASK_QRCODE_LOGGER_H
#define NETWORK_STATUS_TASK_QRCODE_LOGGER_H
...
class QRCodeWriteLogger
{
public:
    QRCodeWriteLogger(bool summaryEnable = true, bool devicesEnable = true);
    ...
    // 封装：按多行结构化格式记录 WriteQRCode 成功
    void logWriteQRCodeSuccess(const QString& deviceId, const ModbusCommand& cmd);
    // 封装：按多行结构化格式记录 WriteQRCode 失败
    void logWriteQRCodeFailure(const QString& deviceId, const ModbusCommand& cmd);
...
private:
    ...
    static QString toHexSpacedUpper(const QByteArray& bytes);
...
#endif // NETWORK_STATUS_TASK_QRCODE_LOGGER_H
