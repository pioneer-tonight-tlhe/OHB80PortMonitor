...
SH85SelfCheckSettingWidget::~SH85SelfCheckSettingWidget()
{
    disconnectTaskSignals();
}
...
void SH85SelfCheckSettingWidget::submitSelfCheckTask(const QString &qrcode)
{
    ...
    if (!task) {
        QMessageBox::warning(this, "Self-check Failed", "SH85 self-check task is not available");
        return;
    }
    ...
    m_taskConnections.append(connect(task, &SH85PeriodicSelfCheckTask2::countdownTick,
                                     this, &SH85SelfCheckSettingWidget::onCountdownTick,
                                     Qt::QueuedConnection));
    m_taskConnections.append(connect(task, &SH85PeriodicSelfCheckTask2::statusChanged,
                                     this, &SH85SelfCheckSettingWidget::onStatusChanged,
                                     Qt::QueuedConnection));
    m_taskConnections.append(connect(task, &SH85PeriodicSelfCheckTask2::singleFinished,
                                     this, &SH85SelfCheckSettingWidget::onAllFinished,
                                     Qt::QueuedConnection));

    QMetaObject::invokeMethod(task, "runSingleDeviceOnce",
                              Qt::QueuedConnection,
...
void SH85SelfCheckSettingWidget::disconnectTaskSignals()
{
    for (const QMetaObject::Connection &connection : qAsConst(m_taskConnections)) {
